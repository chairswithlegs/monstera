...
description: >
  Deep subject matter expertise on the ActivityPub protocol, ActivityStreams 2.0 vocabulary,
  and federated social networking. Use this skill whenever the user is working on anything
  related to ActivityPub: implementing Actors, handling inbox/outbox logic, federating with
  Mastodon or other servers, debugging HTTP Signatures, Webfinger lookups, JSON-LD context
  issues, delivery failures, or any federation interop problem. Also trigger for questions
  about the W3C ActivityPub spec, ActivityStreams types, or fediverse conventions even if
  the user doesn't use the word "ActivityPub" explicitly — e.g. "why won't Mastodon follow
  me back", "my inbox isn't receiving posts", "signature verification is failing", or
  "how do I implement a Follow/Accept flow".
---
