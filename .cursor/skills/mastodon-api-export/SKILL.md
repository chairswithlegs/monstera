---
name: mastodon-api-expert
description: >
  Deep subject matter expertise on implementing a Mastodon-compatible REST API in Go.
  Use this skill whenever the user is working on anything related to the Mastodon API:
  OAuth app registration and token flows, statuses (create/boost/favourite), accounts and
  follow relationships, timelines (home/public/hashtag/list), streaming (SSE and WebSocket),
  media attachments, notifications, or any Mastodon client compatibility issue. Also trigger
  for questions like "why won't Ivory/Tusky/Elk connect to my server", "what fields does
  Mastodon return for a status", "how do I implement the streaming API", or any question
  about making a third-party Mastodon client work against a custom backend — even if the
  user doesn't explicitly say "Mastodon API".
---
