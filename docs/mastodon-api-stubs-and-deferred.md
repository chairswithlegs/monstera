...
|------|------------------|--------|
| **Status card (link preview)** | `card` field on Status entity | Plan §1.2: "optional later: status card (link preview)". Roadmap §13: Link preview requires OG parsing, optional `status_cards` table, and a background worker; deferred. Currently `card` is `null` on all statuses. |
...
- **Deferred behavior:** Status card (link preview), translate — documented as optional/later; no stub endpoints added.
