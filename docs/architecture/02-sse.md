...
3. **Hub** (`internal/api/mastodon/sse/hub.go`): `Subscribe(streamKey)` creates a channel for this client and, if needed, subscribes to the corresponding NATS subject. The handler reads from the channel and writes SSE frames (`event: <type>\ndata: <payload>\n\n`).
4. **Keepalive**: Every 30 seconds the server sends a comment line `:keepalive\n\n` to prevent proxy timeouts.
5. **Publishing**: Domain events flow through the transactional outbox (see [06-domain-events-and-outbox.md](06-domain-events-and-outbox.md)). The **SSE subscriber** (`internal/api/mastodon/sse/subscriber.go`) consumes from the `domain-events-sse` NATS JetStream consumer on the `DOMAIN_EVENTS` stream and translates events into Mastodon API JSON. It publishes `SSEEvent` messages to NATS core subjects based on visibility: public, public:local, per-follower user streams, per-hashtag streams, per-list streams, and per-user direct streams. The Hub receives the NATS message, decodes the event, and sends it to all subscribers for that stream key.
...
| `list:{listID}` | `events.list.{listID}` | List timeline |
| `direct:{accountID}` | `events.direct.{accountID}` | Direct messages for that account |

Constants and mapping live in `internal/api/mastodon/sse/event.go` (`StreamKeyToSubject`, `SubjectToStreamKey`).
...
| `internal/api/mastodon/sse/hub.go` | Subscribe/Unsubscribe; NATS subscriptions (always-on for public/public:local; on-demand for user/hashtag/list/direct); fan-out to channels. |
| `internal/api/mastodon/sse/event.go` | `SSEEvent` struct; stream key ↔ NATS subject mapping. |
| `internal/api/mastodon/sse/subscriber.go` | Consumes domain events from `DOMAIN_EVENTS` JetStream; translates to Mastodon API JSON and publishes to NATS core subjects for the Hub. |
