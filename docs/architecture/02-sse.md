...
5. **Publishing**: When a status is created, deleted, or a notification is created, the service layer calls `EventBus` (implemented by `internal/events/sse/mastodon/publisher.go`). The publisher builds an `SSEEvent` (stream, event type, JSON data), serializes it, and publishes to NATS. For status create it publishes to public, public:local, per-follower user streams, and per-hashtag streams. The Hub receives the NATS message, decodes the event, and sends it to all subscribers for that stream key.
...
| `internal/events/sse/mastodon/publisher.go` | Implements `events.EventBus` and ActivityPub `InboxEventPublisher`; publishes to NATS on status create/delete and notification create. |
