...
- **SSE subscriber** (`internal/events/sse/subscriber.go`): Translates domain events into Mastodon API JSON and publishes to NATS core subjects. A per-process Hub (`internal/events/sse/hub.go`) subscribes to these subjects and fans them out to connected SSE clients. See [02-sse.md](02-sse.md).

This architecture means the service layer is fully decoupled from both federation and SSE. See [06-domain-events-and-outbox.md](06-domain-events-and-outbox.md) for details.
