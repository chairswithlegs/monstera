...
func newInboxProcessorForTest(t *testing.T, fake *testutil.FakeStore, cfg *config.Config) Inbox {
	...
	return NewInbox(accountSvc, followSvc, notificationSvc, statusSvc, statusWriteSvc, mediaSvc, nil, cacheStore, bl, cfg)
}
...
func TestInbox_Process_UpdatePerson_forgedActorRejected(t *testing.T) {
	...
	objectActor := map[string]string{"id": testBobAPID, "type": objectTypePerson, "preferredUsername": "bob"}
...
func TestInbox_syncRemoteActorFromDoc_sanitizesFields(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	fake := testutil.NewFakeStore()
	cfg := &config.Config{InstanceDomain: "example.com"}
	...
	actor := &Actor{
		ID:                "https://bob.com/users/bob",
		Type:              objectTypePerson,
		PreferredUsername: `bob<script>evil()</script>`,
		Name:              `Bob <script>alert(1)</script> <b>Smith</b>`,
		Summary:           `bio <script>bad()</script> text with <a href="https://example.com">link</a>`,
		Inbox:             "https://bob.com/users/bob/inbox",
		Outbox:            "https://bob.com/users/bob/outbox",
		Followers:         "https://bob.com/users/bob/followers",
		Following:         "https://bob.com/users/bob/following",
		PublicKey:         PublicKey{ID: "https://bob.com/users/bob#main-key"},
	}

	acc, err := proc.(*inbox).syncRemoteActorFromDoc(ctx, actor)
	require.NoError(t, err)
	require.NotNil(t, acc)

	assert.Equal(t, "bob", acc.Username, "script must be stripped from username")
	require.NotNil(t, acc.DisplayName)
	assert.NotContains(t, *acc.DisplayName, "<script>", "script must be stripped from display name")
	assert.Contains(t, *acc.DisplayName, "<b>", "safe bold tag must be preserved in display name")
	require.NotNil(t, acc.Note)
	assert.NotContains(t, *acc.Note, "<script>", "script must be stripped from bio")
	assert.Contains(t, *acc.Note, "<a href=", "safe links must be preserved in bio")
}
