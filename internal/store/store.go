...
type Store interface {
	...
	// Trending index — source queries (used by the scheduled job)
	GetTopScoredPublicStatuses(ctx context.Context, since time.Time, limit int) ([]domain.TrendingStatus, error)
	GetHashtagDailyStats(ctx context.Context, since time.Time) ([]HashtagDailyStats, error)

	// Trending index — writes (used by the scheduled job)
	ReplaceTrendingStatuses(ctx context.Context, entries []TrendingStatusEntry) error
	UpsertTrendingTagHistory(ctx context.Context, entries []TrendingTagHistoryEntry) error

	// Trending index — reads (used by TrendsService)
