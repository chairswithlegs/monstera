...
const cancelAccountDeletion = `-- name: CancelAccountDeletion :execrows
...
func (q *Queries) CancelAccountDeletion(ctx context.Context, id string) (int64, error) {
	result, err := q.db.Exec(ctx, cancelAccountDeletion, id)
	if err != nil {
		return 0, err
	}
...
const deleteAccountIfDeletionPending = `-- name: DeleteAccountIfDeletionPending :execrows
...
func (q *Queries) DeleteAccountIfDeletionPending(ctx context.Context, id string) (int64, error) {
	result, err := q.db.Exec(ctx, deleteAccountIfDeletionPending, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const listLocalAccountsPastDeletionGrace = `-- name: ListLocalAccountsPastDeletionGrace :many
...
type ListLocalAccountsPastDeletionGraceParams struct {
	DeletionRequestedAt pgtype.Timestamptz `json:"deletion_requested_at"`
	Limit               int32              `json:"limit"`
}

func (q *Queries) ListLocalAccountsPastDeletionGrace(ctx context.Context, arg ListLocalAccountsPastDeletionGraceParams) ([]string, error) {
	rows, err := q.db.Query(ctx, listLocalAccountsPastDeletionGrace, arg.DeletionRequestedAt, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const requestAccountDeletion = `-- name: RequestAccountDeletion :execrows
UPDATE accounts SET deletion_requested_at = NOW(), suspended = TRUE, updated_at = NOW()
WHERE id = $1 AND deletion_requested_at IS NULL
`

func (q *Queries) RequestAccountDeletion(ctx context.Context, id string) (int64, error) {
	result, err := q.db.Exec(ctx, requestAccountDeletion, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
