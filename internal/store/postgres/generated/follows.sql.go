...
type GetUnbackfilledRemoteFollowingParams struct {
	AccountID        string             `json:"account_id"`
	LastBackfilledAt pgtype.Timestamptz `json:"last_backfilled_at"`
	Limit            int32              `json:"limit"`
}

type GetUnbackfilledRemoteFollowingRow struct {
	Account Account `json:"account"`
}

func (q *Queries) GetUnbackfilledRemoteFollowing(ctx context.Context, arg GetUnbackfilledRemoteFollowingParams) ([]GetUnbackfilledRemoteFollowingRow, error) {
	rows, err := q.db.Query(ctx, getUnbackfilledRemoteFollowing, arg.AccountID, arg.LastBackfilledAt, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetUnbackfilledRemoteFollowingRow{}
	for rows.Next() {
		var i GetUnbackfilledRemoteFollowingRow
		if err := rows.Scan(
			&i.Account.ID,
			&i.Account.Username,
			&i.Account.Domain,
