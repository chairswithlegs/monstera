...
const (
	...
	AvatarMaxDimension = 400  // avatars are square-fit to 400×400
	HeaderMaxWidth     = 1500 // headers fit within 1500×500
	HeaderMaxHeight    = 500
)
...
// ResizeForAvatar resizes img to fit within AvatarMaxDimension×AvatarMaxDimension,
// preserving aspect ratio. No-ops if the image is already within the limit.
func ResizeForAvatar(img image.Image) image.Image {
	b := img.Bounds()
	if b.Dx() <= AvatarMaxDimension && b.Dy() <= AvatarMaxDimension {
		return img
	}
	return imaging.Fit(img, AvatarMaxDimension, AvatarMaxDimension, imaging.Lanczos)
}

// ResizeForHeader resizes img to fit within HeaderMaxWidth×HeaderMaxHeight,
// preserving aspect ratio. No-ops if the image is already within the limit.
func ResizeForHeader(img image.Image) image.Image {
	b := img.Bounds()
	if b.Dx() <= HeaderMaxWidth && b.Dy() <= HeaderMaxHeight {
		return img
	}
	return imaging.Fit(img, HeaderMaxWidth, HeaderMaxHeight, imaging.Lanczos)
}
