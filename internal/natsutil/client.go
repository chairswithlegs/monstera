...
//
// Conn and JS are exported as concrete types rather than behind an interface.
// This is a deliberate choice: NATS is a core infrastructure dependency and
// the nats-go SDK types (nats.Conn, jetstream.JetStream) are already stable,
// well-tested interfaces in practice. Wrapping them in our own interface would
// add indirection without meaningful testability gains — callers that need
// test isolation use natsutil.Publisher, natsutil.ConnSubscriber, or
// accept jetstream.JetStream directly (which is itself an interface).
//
// Conn is used by: SSE hub (pub/sub via ConnSubscriber), SSE subscriber
// (as natsutil.Publisher), and the health checker (connection status).
// JS is used by consumers and publishers throughout the application.
type Client struct {
	Conn *nats.Conn
