...
// subscriberNATSPublisher is the subset of *nats.Conn used for publishing SSE
// events to NATS core subjects (consumed by the Hub for real-time fan-out).
type subscriberNATSPublisher interface {
...
func (s *Subscriber) Start(ctx context.Context) error {
	consumer, err := s.js.Consumer(ctx, events.StreamDomainEvents, events.ConsumerSSE)
	if err != nil {
		return fmt.Errorf("sse subscriber: get consumer: %w", err)
	}

	slog.Info("sse subscriber started",
		slog.String("consumer", events.ConsumerSSE),
	)
	...
	consCtx, err := consumer.Consume(
		...
		jetstream.ConsumeErrHandler(func(_ jetstream.ConsumeContext, err error) {
			if ctx.Err() == nil {
				slog.Warn("sse subscriber consume error", slog.Any("error", err))
			}
		}),
	)
	if err != nil {
		return fmt.Errorf("sse subscriber: consume: %w", err)
	}
...
func (s *Subscriber) publish(_ context.Context, subject string, ev SSEEvent, metricSubject string) {
	...
	if err != nil {
		...
		s.metrics.NATSPublishTotal.WithLabelValues(metricSubject, "error").Inc()
		return
	}
	...
	s.metrics.NATSPublishTotal.WithLabelValues(metricSubject, "ok").Inc()
}
