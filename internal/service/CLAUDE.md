...
- The base verb (`Create`, `Update`, `Delete`) is the **local** path: increments counts, federates, sends notifications.
- The `*Remote` variant handles federated-in objects: no outbound federation, different count semantics (e.g. no account statuses count increment for remote statuses).
