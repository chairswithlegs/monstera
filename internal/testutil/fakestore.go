...
// SeedStatus inserts a status directly into the FakeStore for test setup.
// CreatedAt is set to now if zero.
func (f *FakeStore) SeedStatus(s *domain.Status) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *FakeStore) GetTrendingStatusIDs(_ context.Context, limit int) ([]domain.TrendingStatus, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := append([]domain.TrendingStatus{}, f.TrendingStatuses...)
	if len(out) > limit {
		out = out[:limit]
	}
	return out, nil
...
func (f *FakeStore) GetTrendingTags(_ context.Context, _ int, limit int) ([]domain.TrendingTag, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	// Build TrendingTag entries from TrendingTagHistory
	type tagEntry struct {
		hashtag   domain.Hashtag
		totalUses int64
		history   []domain.TagHistoryDay
	}
	byID := make(map[string]*tagEntry)
	for _, e := range f.TrendingTagHistory {
		te, ok := byID[e.HashtagID]
		if !ok {
			h, exists := f.hashtagsByID[e.HashtagID]
			if !exists {
				h = &domain.Hashtag{ID: e.HashtagID, Name: e.HashtagID}
			}
			te = &tagEntry{hashtag: *h}
			byID[e.HashtagID] = te
		}
		te.totalUses += e.Uses
		te.history = append(te.history, domain.TagHistoryDay{Day: e.Day, Uses: e.Uses, Accounts: e.Accounts})
	}
	out := make([]domain.TrendingTag, 0, len(byID))
	for _, e := range byID {
		out = append(out, domain.TrendingTag{Hashtag: e.hashtag, History: e.history})
	}
	if len(out) > limit {
		out = out[:limit]
	}
...
func (f *FakeStore) UpsertStatusCard(_ context.Context, in store.UpsertStatusCardInput) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.StatusCards[in.StatusID] = &domain.Card{
		StatusID:        in.StatusID,
		ProcessingState: in.ProcessingState,
		URL:             in.URL,
		Title:           in.Title,
		Description:     in.Description,
		Type:            in.CardType,
...
// GetStatusCard returns the card for the given status, or domain.ErrNotFound.
func (f *FakeStore) GetStatusCard(_ context.Context, statusID string) (*domain.Card, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	c, ok := f.StatusCards[statusID]
	if !ok {
		return nil, domain.ErrNotFound
	}
	copy := *c
...
func (f *FakeStore) InsertOutboxEvent(_ context.Context, in store.InsertOutboxEventInput) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.OutboxEvents = append(f.OutboxEvents, domain.DomainEvent{
		ID:            in.ID,
...
func (f *FakeStore) GetAndLockUnpublishedOutboxEvents(_ context.Context, limit int) ([]domain.DomainEvent, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if limit > len(f.OutboxEvents) {
		limit = len(f.OutboxEvents)
	}
	return f.OutboxEvents[:limit], nil
}

func (f *FakeStore) MarkOutboxEventsPublished(_ context.Context, ids []string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
