...
- Content input: plain text only. Strip HTML, auto-link URLs/@mentions/#hashtags.
...
- Handler file decomposition: large handlers are split into `{resource}_{concern}.go` files (e.g. `statuses_actions.go`).