...
func (h *StatusesHandler) GETContext(w http.ResponseWriter, r *http.Request) {
	...
	enrichedAncestors, err := h.statuses.GetByIDsEnriched(r.Context(), ancestorIDs, viewerID, domain.FilterContextThread)
	if err != nil {
		api.HandleError(w, r, err)
		return
	}
	...
	enrichedDescendants, err := h.statuses.GetByIDsEnriched(r.Context(), descendantIDs, viewerID, domain.FilterContextThread)
	if err != nil {
		api.HandleError(w, r, err)
		return
	}
