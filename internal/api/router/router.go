	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				r.Get("/accounts/{id}/statuses", deps.Accounts.GETAccountStatuses)
				r.Get("/accounts/{id}/followers", deps.Accounts.GETFollowers)
				r.Get("/accounts/{id}/following", deps.Accounts.GETFollowing)
			...
			r.Group(func(r chi.Router) {
				...
				r.Method("GET", "/domain_blocks", middleware.RequiredScopes("read:blocks")(http.HandlerFunc(deps.Accounts.GETDomainBlocks)))
				r.Method("POST", "/domain_blocks", middleware.RequiredScopes("write:blocks")(http.HandlerFunc(deps.Accounts.POSTDomainBlock)))
				r.Method("DELETE", "/domain_blocks", middleware.RequiredScopes("write:blocks")(http.HandlerFunc(deps.Accounts.DELETEDomainBlock)))
